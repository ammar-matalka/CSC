...
<div class="max-w-4xl">
    <form action="{{ isset($solution) ? route('admin.solutions.update', $solution) : route('admin.solutions.store') }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-2xl shadow-lg p-8" x-data="{ featureCount: {{ isset($solution) && $solution->features ? count($solution->features) : 2 }} }">
        ...
        <div class="mb-6">
            ...
            <input type="text" name="icon" value="{{ old('icon', $solution->icon ?? 'fas fa-laptop-code') }}" class="w-full px-4 py-3 rounded-lg border border-gray-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none transition" required>
        </div>
